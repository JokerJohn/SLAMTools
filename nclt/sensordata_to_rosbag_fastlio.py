...
def write_groundtruth():
    odom_gt = np.loadtxt(sys.argv[1] + "groundtruth.csv", delimiter = ",")
    print(len(odom_gt))
...
def write_ms25(ms25, ms25_euler, i, bag):
    ...
    r_q = R.from_euler('zyx', [0, 0, 0], degrees=False)
    q = R.from_euler('zyx', [0, 0, 0], degrees=False).as_quat()
    ...
    while i < data_lenth :
        ...
        if i > 0 :
            ...
            r_q = R.from_euler('xyz', [h, p, r], degrees=False)  # 创建新的Rotation对象
        ...
        # r_q = R.r = R.from_euler('xyz', [h, p, r], degrees=False)
