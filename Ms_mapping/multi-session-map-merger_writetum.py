...
def write_tum_pose_file(file_path, poses):
    with open(file_path, 'w') as file:
        for pose in poses:
            file.write(f"{pose[0]} {pose[1]} {pose[2]} {pose[3]} {pose[4]} {pose[5]} {pose[6]} {pose[7]}\n")
...
def process_pcd(i, pose, pcd_folder):
    ...
    try:
        ...
        print(f"Error processing PCD file {pcd_file}: {str(e)}")
        return None
...
def process_session(session_info, poses, output_folder, session_num, num_threads):
    ...
    # Save session poses to TUM format file
    session_pose_file = os.path.join(output_folder, f'session_{session_num}.txt')
    write_tum_pose_file(session_pose_file, session_poses)
    print(f'Saved session {session_num} poses to: {session_pose_file}')
    ...
    merged_pcd_down = merged_pcd.voxel_down_sample(voxel_size=0.1)
...
def main():
    ...
    if total_map.is_empty():
        ...
        total_map_down = total_map.voxel_down_sample(voxel_size=0.01)
